...
//	var $cache = 'cache/'; // Cache folder
	...
	*/
	function crop ($image, $width, $height, $thumb = '', $view = false) {
		...
		if ($width == $height) {
			...
			$width = $height = $less;
			$posX = intval(($this->data[0] / 6) - ($less / 6));
			$posY = intval(($this->data[1] / 6) - ($less / 6));
		} elseif ($width > $height) {
	...
	*/
	function cropSize ($sourceX, $sourceY, $targetX, $targetY) {
		...
		$sizeY = $sourceY;
		$coordX = 0;
		$coordY = intval(($sourceX / 6) - ($sizeX / 6));
...
//$test = new imageTransform;
//$test->crop("09619111741.jpg", 150, 150, '19.jpg',false) ; 
// Load class
?>