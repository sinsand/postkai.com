<div class="row pb-10">
  <div class="col-xs-12 text-right pb-10">
    <a href="<?php echo $LinkWeb;?>post-new" class="btn btn-lg btn-success"><?php echo $translations["mem-fetch-post-head"];?> <i class="fa fa-plus"></i></a>
  </div>
</div>
<?php
 $SqlSelect = "SELECT *
   ...
   foreach (select_tb($SqlSelect) as $row) {
     ?>
      <div class="row pb-10 pt-10" style="border-bottom: 1px solid #e1e1e1;">
        <div class="col-xs-6 col-sm-2 col-md-2 text-left fw-b">#<?php echo $translations["mem-fetch-post-no"]." ".$row['IDJOB'];?></div>
        <div class="col-sm-7 hidden-xs"><b>#<?php echo $translations["mem-fetch-post-subject"];?></b><div style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis;"><?php echo $row['jTitle'];?></div></div>
        ...
        <div class="col-xs-12 pt-5 hidden-sm hidden-md hidden-lg"><b>#<?php echo $translations["mem-fetch-post-subject"];?></b> <?php echo $row['jTitle'];?></div>
      </div>
     <?php
   }
 }else {
